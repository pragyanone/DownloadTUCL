...
# Step 1: Download all pages first
for page_num in range(1, 8):
    ...
    # Write original HTML content to file
    with open(page_filename, "w", encoding="utf-8") as f:
        f.write(str(page_soup))
    print(f"Downloaded page {page_num}")

# Step 1b: Modify each page to have proper links to other pages in the page number widget
for page_num in range(1, 8):
    page_filename = f"tucl/page_{page_num}.html"
    ...
    for link in page_links:
        link['href'] = f"tucl/page_{link.text.strip().replace('(current)', '').strip()}.html"
    # Write modified HTML content to file
    with open(page_filename, "w", encoding="utf-8") as f:
        f.write(str(page_soup))
    print(f"Updated links in page {page_num}")
    ...
    for link in paper_links:
        ...
        if link_text != "No Thumbnail Available":
            if os.path.exists(paper_filename):
                ...
                print(f"Downloaded: Page-{page_num} Paper-{paper_counter + 1} {paper_filename}")
                ...
                for pdf_link in pdf_links:
                    ...
                    with open(paper_filename, "r", encoding="utf-8") as f:
                        html_content = f.read()
                        html_content = html_content.replace(pdf_link["href"], pdf_relative_path)
                    with open(paper_filename, "w", encoding="utf-8") as f:
                        f.write(html_content)
            paper_counter += 1  # Increment the counter here
...
for page_num in range(1, 8):
    ...
    with open(page_filename, "w", encoding="utf-8") as f:
        f.write(str(page_soup))
    print(f"Updated links in page {page_num} to point to the correct paper-pages")
